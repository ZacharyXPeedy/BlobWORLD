...
const hook = new Webhook("https://discord.com/api/webhooks/1394240599795830874/hEe7RviGHNSYe3Y00w5LlmYmwKZCnpvcqiL4KMwZFFXP4O2dbMz6F-fp9-PNC1yiZ-QI");
//Read settings
...
server.listen(config.port, () => {
    ...
    console.log("running at http://blob.localhost:" + config.port);
});
...
var commands = {
  ...
  moron:(victim, param)=>{
    victim.room.emit("talk",{guid:victim.public.guid, text:"You are a moron!"});
    victim.lastMessage = "You are a moron!";
  },
  ...
  image: (victim, param) => {
    if (!param.startsWith("https://files.catbox.moe")) {
      victim.room.emit("talk", {
        guid: victim.public.guid,
        text: "Only use Catbox.moe links."
      });
      return;
    }
  ...
  audio: (victim, param) => {
    ...
    victim.room.emit("talk", {
      guid: victim.public.guid,
      text: `<audio class='useraudio' src='${param.replace(/'/g, "&apos;")}' controls></audio>`
    });
  },
  ...
  video: (victim, param) => {
    ...
    victim.room.emit("talk", {
      guid: victim.public.guid,
      text: `<video class='uservideo' src='${param.replace(/'/g, "&apos;")}' controls></video>`
    });
  },
  ...
  floyd:(victim, param)=>{
    if(victim.level<1.1 || !victim.room.usersPublic[param]) return;
    users[param].muted = 2;
    victim.room.usersPublic[param].name = "DIRTY BAD BOY";
    victim.room.usersPublic[param].dispname = "DIRTY BAD BOY";
    victim.room.usersPublic[param].color = "brown";
    victim.room.usersPublic[param].tagged = true;
    victim.room.usersPublic[param].tag = "DIRTY BAD BOY";
    victim.room.usersPublic[param].typing = "";
    victim.room.emit("update",{guid:param,userPublic:victim.room.usersPublic[param]});
    users[param].socket.emit("nuke");
    victim.lastMessage = "I am a bitch! I am so bad fuck!";
    if (users[param].nuked == null)
      users[param].nuked = setInterval(() => {
        victim.room.emit("talk", { guid: param, text: "I am a bitch! I am so bad fuck!"})
      }, 1200);
  },
        ...
        this.socket.on("talk", (msg) => {
          ...
          //talk
            if(!this.slowed && this.timesSent != config.spamlimit){
              ...
              // erm how am i gonna make a webhook
              // coded by anony???????????????????????????
              var text = msg.text;
              var rid = this.room.name;
